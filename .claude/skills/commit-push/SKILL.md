...
description: |
  ...
  Boundary: 커밋 생성과 작업 브랜치 push 까지만. PR 생성, 머지, 리뷰는 범위 밖이다.
allowed-tools: Bash(git *), Read
---

# 커밋, 푸시

변경 사항을 커밋하고 작업 브랜치에 push 한다. 산출물(리뷰 패킷, build-state)은 참고용으로만 보고
...
1. `git branch --show-current` 로 현재 브랜치를 확인하라.
2. **현재 브랜치가 `main` 이면 중단하라.** "main 에 직접 커밋하지 않는다"를 알리고,
   `build-state.json` 의 현재 `IN_PROGRESS` 항목으로 브랜치명을 제안하라
   (`{type}/{항목ID의 콜론을 하이픈으로}` - 예: `feat/step-6-expand`). → [[git-convention]]
3. `git status --short` 로 스테이징, 미스테이징, untracked 를 파악하라.
4. `git diff` 와 `git diff --staged` 로 **실제 변경 내용을 직접 확인하라.**
5. 커밋할 변경이 없으면 알리고 중단하라.

> 다음 Phase 조건: 변경 내용과 브랜치를 파악했을 때
> Skip 조건: 없음
...
3. 그룹마다 `{type}: 내용` 초안을 작성하라.
   - **40자 이내 명사형.** 범위 표기(`feat(step-5):`)와 괄호를 쓰지 않는다
...
1. 커밋 그룹, 메시지 초안, 푸시 대상(브랜치, 첫 푸시 여부)을 제시하고 "이렇게 커밋, 푸시할까요?"로 확인받아라.
