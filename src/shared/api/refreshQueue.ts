...
export function installRefreshInterceptor(): void {
  apiClient.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      /**
       * 재발급 요청 자체가 401 이면 여기서 되살릴 방법이 없다. 다만 **여기서 로그아웃시키지 않는다.**
       * 부른 쪽이 정리한다 — 아래 큐가 부른 경우는 catch 가, protectedLoader 가 직접 부른 경우는
       * 로더가 처리한다. 둘 다 하면 하드 이동과 라우터 이동이 경쟁해서 만료 안내가 사라진다.
       */
