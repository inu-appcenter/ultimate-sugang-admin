...
절 번호가 곧 주소입니다. **행은 아래 명령이 줍니다. spec 파일을 통째로 읽지 마세요**(`03`·`04` 는 각 1,000줄이 넘습니다).
...
⚠️ 리뷰 시점에는 `gate-runner.sh --full` 이 이미 green 입니다. **위 항목을 "확인했다"며 다시 훑는 것이 이 리뷰의 가장 큰 낭비였습니다.**
...
⚠️ **설명하려고 든 예시를 요구사항으로 키우지 않습니다.** "다른 관리자가 동시에 …" 같은 상황이 명세 절에 매핑되지 않으면 지적이 아니라 `QUESTION(🙋🏻)` 입니다. 예시 → 시나리오 → 요구사항으로 부풀린 항목 하나가 한 세션에서 가장 오래 붙잡은 작업이 됐고, 결국 명세에 없는 동작이었습니다.
...
**한 항목당 한 번만 호출됩니다.** 재리뷰는 사용자가 따로 요청할 때만 있습니다 — 다음 라운드에서 마저 보겠다는 전제로 지적을 아끼지 마세요.
