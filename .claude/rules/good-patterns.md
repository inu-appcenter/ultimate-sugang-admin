...
`src/` 에 주석을 남기지 않는다. 이름과 구조로 설명한다.

- ❌ 코드가 이미 말하는 것을 한국어로 다시 쓰기 (`// 토큰을 저장한다` 위의 `tokenManager.set(...)`)
- ❌ `// ── 인증 ──` 같은 구역 배너. 파일이 길어서 배너가 필요하면 파일을 쪼갠다
- ❌ 명세 인용(`(01 §6-4)`, `(D10)`)을 코드에 박기 → 아래 "근거는 어디에 남기나"
- ❌ JSDoc 설명문. 타입이 시그니처에 이미 있다
- ❌ `// TODO`·`// Step 5 에서 붙인다` → 할 일은 `build-state.json` 의 체크리스트가 갖는다

**남겨도 되는 것은 둘뿐이다.**
1. **컴파일러·도구 지시자** — `/// <reference types="vite/client" />`, 불가피한 `eslint-disable`. 지우면 동작이 바뀐다
2. 없음. 1번 말고는 없다
