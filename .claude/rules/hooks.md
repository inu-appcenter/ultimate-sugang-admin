...
- `hooks/checks/` — 진입점이 부르는 **검사**: `gate-runner.sh`(묶음) · `validate-state.mjs` · `spec-presence.mjs` · `typecheck.sh` · `token-lint.mjs` · `uss-contract-lint.mjs` · `build.sh` · `smoke.sh` · `lint.sh`
- `harness/verify/` — **동작 검증**(jsdom). `--full` 에서 돈다. 무엇을 넣고 무엇을 넣지 않는지는 아래 §verify.
- `hooks/checks/d4-strategy.mjs` — D4 정규식. `uss-contract-lint`(검사)와 `harness/verify/step-2.mjs`(그 규칙이 잡아야 할 것 고정)가 **같은 파일을 공유**한다. 규칙을 고치면 양쪽에 동시 반영된다.
...
- **`--full`** = fast + **`verify`**(`npm run verify` — jsdom 동작 검증, 약 30초) + `build`(vite). **커밋과 리뷰 패킷 직전**에 오케스트레이터가 돌린다.
...
> `spec-presence` 는 묶음에 **없다.** 설치 실수는 빌드를 막을 일이 아니라 알려주면 되는 일이다.
> 수동 실행: `node .claude/hooks/checks/spec-presence.mjs`
...
- **verify**(`npm run verify`) — **눈으로 확인할 수 없는 동작만** 본다. 파일은 4개다.
  - `step-2` 계약·401 재발급 큐·D10 캐시 격리·D4 가드 / `step-5-3` 실행 202 와 409 2종 / `step-5-4` 폴링 / `step-6` 확장 단일성·상세 쿼리·종료 시 자동 확장.
  - ❌ 마크업 존재·문구·색·배지·빈 상태·모달 레이아웃을 넣지 않는다. 브라우저에서 보면 되는 것이고, 문자열 매칭이라 UI 를 한 줄 고칠 때마다 깨진다. 시각 회귀는 Step 7 Playwright 스모크가 본다.
  - ❌ 소스를 문자열로 훑는 규칙을 넣지 않는다. 그건 `uss-contract-lint` 다 — verify 에 넣으면 fast 게이트에서 안 돌아 매 턴 방어가 사라진다.
  - 폴링 간격은 `VITE_POLL_INTERVAL_MS`(기본 2000)로 주입한다. `harness/verify/env.mjs` 가 40ms 로 덮어 실시간 대기를 없앤다 — `wait` 값을 다시 초 단위로 늘리지 않는다.
...
- `build-state.json` 의 **`retry`·`probes` 는 항목 ID 를 키로 쓰는 객체**(`{"step-3:ADMIN_LOGIN": 2}`)이고 `manual_review`·`deferred` 는 배열이다. 숫자나 문자열로 바꾸면 3회 한도가 동작하지 않는다.
  ...
  - **`probes` 는 원인 탐색 횟수**다. 3회를 넘기면 `notes` 에 `unresolved` 로 적고 넘어간다.
